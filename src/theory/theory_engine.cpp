...
#define CVC5_FOR_EACH_THEORY_STATEMENT(THEORY)                               \
  ...
  if (options().theory.originTracker)
  {
    d_origTracker.reset(new OriginTracker(d_env, this));
    d_modules.push_back(d_origTracker.get());
  }
    ...
    const std::vector<Node>& assertions)
{
  ...
  if (d_origTracker)
  {
    d_origTracker->notifyPreprocessedAssertions(assertions);
  }
}
...
std::optional<theory::OriginTracker::Origin> TheoryEngine::getOrigin(
    Node n) const
{
  // if there is no origin tracker, we fail
  return d_origTracker ? d_origTracker->getOrigin(n) : std::nullopt;
}
